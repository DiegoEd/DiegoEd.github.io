...
class ValidadorRegistro {
	...
	private function validar_nombre($nombre, $conexion) {
		...
		if (strlen($nombre) < 6) {
			return "El nombre debe ser más largo que 6 c.aracteres.";
		}
		if (strlen($nombre) > 24) {
			return "El nombre no puede ocupar más de 24 caracteres.";
		}
		if (RepositorioUsuario::nombre_existe($nombre, $conexion)) {
	...
	private function validar_email($email, $conexion) {
		...
		if (RepositorioUsuario::email_existe($email, $conexion)) {
			return "Este email ya está en uso. Por favor, pruebe otro email o <a href='#'>intente rescuperar su contraseña</a>.";
		}
		return "";
	}
